...
public class forget2 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        e=findViewById(R.id.forgetpassword);
        b=findViewById(R.id.Resetpassword);
        DatabaseReference refer= FirebaseDatabase.getInstance().getReference(datab);
        b.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String str=e.getEditableText().toString();
                if (!validatePassword()) {
                    return;
                }
                else
                    {
                        refer.child(usern).child("password").setValue(str);
